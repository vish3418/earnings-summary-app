    ...
    try {
        ...
        const data = {
            ...
            overview: {
                ...
                forwardPE: overviewResponse.data.ForwardPE,
                pegRatio: overviewResponse.data.PEGRatio
            },
...
async function generateAISummary(stockData, earningsData) {
    try {
        const prompt = `
        Analyze the following earnings data for ${stockData.name} (${stockData.symbol}):
        ...
        Company Metrics:
        - Revenue (TTM): $${(earningsData.overview.revenue / 1000000000).toFixed(2)}B
        ...
        const response = await axios.post(
            'https://api.openai.com/v1/chat/completions',
            {
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a financial analyst providing concise, insightful earnings summaries.'
                    },
                    {
                        role: 'user',
                        content: prompt
...
function generateTemplateSummary(stockData, earningsData) {
    ...
    const epsBeat = parseFloat(earnings.reportedEPS) > parseFloat(earnings.estimatedEPS);
    const surprisePercent = parseFloat(earnings.surprisePercentage);
    
    let summary = `${stockData.name} (${stockData.symbol}) `;
    
    if (epsBeat) {
        summary += `beat earnings expectations with EPS of $${earnings.reportedEPS} vs. estimated $${earnings.estimatedEPS}, a positive surprise of ${Math.abs(surprisePercent)}%. `;
    } else {
        summary += `missed earnings expectations with EPS of $${earnings.reportedEPS} vs. estimated $${earnings.estimatedEPS}, falling short by ${Math.abs(surprisePercent)}%. `;
    }
    
    if (overview.revenue) {
        summary += `The company reported TTM revenue of $${(overview.revenue / 1000000000).toFixed(2)}B `;
